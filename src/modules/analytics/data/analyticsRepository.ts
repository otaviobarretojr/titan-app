...
export async function getAnalyticsSummary(daysCount = 90): Promise<AnalyticsSummary> {
  ...
  const [plans, meals, hydration, sleep, workouts, cardios, metrics, records, recommendations] = await titanDatabase.transaction('r', [titanDatabase.dailyPlans, titanDatabase.mealEntries, titanDatabase.hydrationEntries, titanDatabase.sleepEntries, titanDatabase.workoutSessions, titanDatabase.cardioSessions, titanDatabase.bodyMetrics, titanDatabase.exercisePersonalRecords, titanDatabase.coachRecommendations], () => Promise.all([inPeriod(titanDatabase.dailyPlans), inPeriod(titanDatabase.mealEntries), inPeriod(titanDatabase.hydrationEntries), inPeriod(titanDatabase.sleepEntries), inPeriod(titanDatabase.workoutSessions), inPeriod(titanDatabase.cardioSessions), inPeriod(titanDatabase.bodyMetrics), titanDatabase.exercisePersonalRecords.where('userId').equals(TITAN_USER_ID).toArray(), inPeriod(titanDatabase.coachRecommendations)]))
