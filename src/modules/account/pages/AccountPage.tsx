...
export function AccountPage() {
  ...
  return <section className="space-y-5" aria-live="polite">
    ...
    <button disabled={!session || offline || busy} className="w-full rounded-3xl bg-emerald-500 p-5 text-lg font-black disabled:opacity-50" onClick={runBackup}>Criar backup agora</button>{stage ? <p className="text-sm text-blue-200">{stage}</p> : null}{message ? <p aria-live="polite" className="rounded-2xl bg-white/10 p-3 text-sm" role="status">{message}</p> : null}
    ...
    {selected && selectedSummary ? <div className="fixed inset-0 z-[80] bg-black/70 p-5"><div aria-describedby="cloud-restore-description" aria-labelledby="cloud-restore-title" aria-modal="true" className="mx-auto max-w-md rounded-3xl bg-slate-900 p-5" role="dialog"><h2 className="text-xl font-black" id="cloud-restore-title">Confirmar substituição?</h2><p className="mt-2 text-sm text-slate-300" id="cloud-restore-description">Backup de {new Date(selectedSummary.exportedAt).toLocaleString()} com {selectedSummary.records} registros. Dados locais atuais: {Object.values(localCounts).reduce((a, b) => a + b, 0)} registros. Esta ação substitui os dados locais após criar snapshot de segurança.</p><div className="mt-4 flex gap-2"><button className="flex-1 rounded-2xl bg-white/10 p-3" onClick={() => setSelected(null)}>Cancelar</button><button disabled={busy} className="flex-1 rounded-2xl bg-red-500 p-3 font-bold disabled:opacity-50" onClick={confirmRestore}>Substituir dados locais</button></div></div></div> : null}
  </section>
}
