...
export async function serializeBackup(options: { includePhotos?: boolean } = {}): Promise<TitanCloudBackup> {
  ...
  const base = {
    ...
    containsPhotos,
    compatibility: { minDatabaseVersion: 1, maxDatabaseVersion: 13, excludedTables: options.includePhotos ? [] : ['progressPhotos'], photosExcludedByDefault: !options.includePhotos, automaticSync: false as const },
  }
